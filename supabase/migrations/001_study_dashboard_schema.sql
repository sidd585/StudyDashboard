...
RETURNS TRIGGER AS $$
DECLARE
    v_role application_role := 'USER';
    v_has_admin BOOLEAN;
    v_display_name TEXT;
    v_avatar TEXT;
BEGIN
    -- Check if any MAIN_ADMIN already exists in system
    SELECT EXISTS (
        SELECT 1 FROM public.profiles WHERE role = 'MAIN_ADMIN'
    ) INTO v_has_admin;

    -- First user or bootstrap user promotion
    IF NOT v_has_admin THEN
        v_role := 'MAIN_ADMIN';
    END IF;

    -- Default display name & avatar
    ...
    -- Insert Starter Sample Courses for normal user
    IF v_role = 'USER' THEN
        INSERT INTO public.courses (user_id, name, description, daily_goal_minutes, color, is_sample)
        VALUES
            (new.id, 'Mathematics (Sample)', 'Arithmetic, Algebra & Problem Solving', 45, '#5b5bd6', TRUE),
            (new.id, 'General Science (Sample)', 'Physics, Chemistry & Biology basics', 45, '#12b76a', TRUE),
            (new.id, 'English Language (Sample)', 'Grammar, Vocabulary & Comprehension', 30, '#0284c7', TRUE)
